...
(defun elisp-test--report-results
  ...
  (let* ((test-names (make-hash-table :test 'equal))
         (report-dir (file-name-directory elisp-test-results-org-path-switched))
         (old-dir (expand-file-name ".old" report-dir))
         ;; Use provided timestamp or generate a new one
         (timestamp (or timestamp (format-time-string "%Y%m%d-%H%M%S")))
         ...
         (total (+ total-passed total-failed total-skipped total-timeout))
         (success-rate
          (if (> total 0)
         ...
         (pdf-file-with-rate
          (replace-regexp-in-string
           ...
           (concat (file-name-sans-extension
                    elisp-test-results-org-path-switched) ".pdf"))))
        ...
        (let ((buf (elisp-test-file-buffer org-file-with-rate)))
          ...
          (when buf
            (with-current-buffer buf
              ...
              (let ((tex-file
                     (concat (file-name-sans-extension org-file-with-rate)
                             ".tex")))
